...
const { loadPlaywright, bootLocal, assert, assertEqual, REPO_ROOT } = require("../harness");
...
module.exports = async function ({ browser, baseUrl, test }) {
  ...
  await test("разбор денег из текста бота: цена не склеивается с числом в скобках", async () => {
    /* Бот разбирает сообщение владельца в позиции сметы, и число из текста
       становится ценой в договоре и счёте. Это самый дорогой разбор в продукте
       и до 04.09.2026 он был не покрыт вовсе.

       Найденная ошибка: первая версия вычищала из строки всё, кроме цифр, и на
       НАСТОЯЩЕЙ строке владельца «Съемка - 50,000₽ (2 оператора)» давала 500002
       вместо 50000 — двойка из «2 оператора» приклеивалась к цене. Модель обязана
       класть уточнение в note и возвращать price числом, но разбор денег не
       должен зависеть от того, послушалась ли она.

       Тест гоняет ЗАДЕПЛОЕННУЮ функцию, вынимая её из .ts: копия в тесте
       разошлась бы с оригиналом молча — ровно то, за чем мы тут гоняемся.
       Набор Deno в офлайновом прогоне не поднять, а функция чистая. */
    const src = require("fs").readFileSync(
      require("path").join(REPO_ROOT, "supabase/functions/parse-estimate/index.ts"), "utf8");
    const m = src.match(/function intOf\([\s\S]*?\n\}/);
    assert(m, "в parse-estimate не найдена функция intOf — разбор денег переехал, тест ослеп");
    // Снимаем типы TS: тело чистое, ни импортов, ни Deno внутри.
    const intOf = new Function("return " + m[0].replace(/:\s*unknown/g, "").replace(/:\s*number/g, ""))();

    const случаи = [
      ["50,000₽ (2 оператора)", 50000, "число в скобках приклеилось к цене"],
      ["135 000₽ + трансфер",  135000, "хвост «+ трансфер» испортил число"],
      ["10,000₽",               10000, "запятая как разделитель тысяч"],
      ["10 000 руб",            10000, "пробел как разделитель тысяч"],
      ["135.000",              135000, "точка как разделитель тысяч"],
      ["1 017 887 ₽",         1017887, "миллионы с пробелами"],
      ["1.234.567",           1234567, "две точки-разделителя"],
      [50000,                   50000, "число пришло числом"],
      ["",                          0, "пустая строка"],
      ["трансфер",                  0, "строка без числа"],
    ];
    for (const [вход, ждём, почему] of случаи) {
      assertEqual(intOf(вход), ждём, `${почему}: intOf(${JSON.stringify(вход)})`);
    }
  });

  await test("бот узнаёт смету в сообщении и не путает её с вопросом", async () => {
    /* Эта проверка решает, разбирать сообщение как смету или отвечать на него
       как на вопрос. Ошибка в одну сторону — сделка молча не создаётся, и
       человек не понимает почему; в другую — вместо ответа показывается разбор.

       Найденное 04.09.2026: требовались «₽», «руб» или «р.» С ТОЧКОЙ, и смета
       «Монтаж 20000р» не распознавалась вовсе. Пишут так постоянно.

       Гоняем ЗАДЕПЛОЕННУЮ функцию, вынимая её из .ts, — копия разошлась бы с
       оригиналом молча. */
    const src = require("fs").readFileSync(
      require("path").join(REPO_ROOT, "supabase/functions/telegram-webhook/index.ts"), "utf8");
    const m = src.match(/function looksLikeEstimate\([\s\S]*?\n  \}/);
    assert(m, "в telegram-webhook не найдена looksLikeEstimate — распознавание переехало, тест ослеп");
    const looksLikeEstimate = new Function("return " + m[0].replace(/:\s*string/g, "").trim())();

    const случаи = [
      ["Создание сценария - 10,000₽\nСъемка - 50,000₽\nИтого: 60,000₽", true, "смета из примера владельца"],
      ["Монтаж 20000р\nГрафика 35000р", true, "«р» без точки — пишут так постоянно"],
      ["Съёмка 50 000 руб\nМонтаж 20 000 руб", true, "«руб» словом"],
      ["Смета на 135 000 рублей\nи ещё 20 000 рублей сверху", true, "«рублей» целиком"],
      ["Сколько должен клиент Альфа?", false, "вопрос про долг уходит в общий ответ"],
      ["Сколько я заработал в этом месяце?", false, "вопрос про деньги — тоже вопрос"],
      ["Съемка - 50 000 руб", false, "одна строка — не смета"],
      ["Нужно 12000 рабочих часов\nи 5000 рабочих мест", false, "«рабочих» — не рубли"],
      ["Привет!\nКак дела?", false, "болтовня"],
    ];
    for (const [текст, ждём, почему] of случаи) {
      assertEqual(looksLikeEstimate(текст), ждём, почему + ": " + JSON.stringify(текст.slice(0, 40)));
    }
  });
