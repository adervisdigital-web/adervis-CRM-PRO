...
module.exports = async function ({ test }) {
  ...
  await test("палитра DESIGN.md совпадает со style.css", () => {
    ...
    const stripComments = (t) => t.replace(/(?<![\w"'/])\/\*[\s\S]*?\*\//g, "");
  ...
  /* Клиент оплатил аванс по ссылке КП — деньги пришли на счёт студии, но в
     ...
     оставалось нулём, долг — полной суммой, выручка месяца не росла,
     собираемость занижалась. Касса и учёт расходились ровно на аванс.

     Проверяем три вещи, каждая со своей ценой ошибки: идемпотентность (иначе
     задвоим деньги — сегодня же на этом обожглись со сметой), сумму из БД (а
     не из нарисованного на экране) и правило этапа (только вперёд). */
  await test("оплаченный аванс вносится в финансы: один раз, суммой из базы", () => {
    const app = fs.readFileSync(path.join(REPO_ROOT, "app.js"), "utf8");
    const from = app.indexOf("async function addAdvancePaymentFromPortal");
    assert(from > 0, "оплаченный клиентом аванс никак не попадает в финансы сделки");
    const fn = app.slice(from, from + 2200);

    assert(
      /portalAdvanceId === portalId/.test(fn),
      "нет защиты от повторного внесения: второй заход на вкладку задвоит деньги"
    );
    assert(
      /portalAdvanceId: portalId/.test(fn),
      "платёж не помечается id КП — значит защита выше не сработает никогда"
    );
    assert(
      /\.from\('client_portals'\)[\s\S]{0,200}advance_amount/.test(fn),
      "сумма аванса берётся не из базы"
    );
    assert(
      /if \(error \|\| !data \|\| !data\.advance_paid_at/.test(fn),
      "платёж создаётся даже при неудачном чтении или неоплаченном авансе"
    );
    assert(
      /CRM_STATUSES\.indexOf\("Предоплата"\)/.test(fn) && /now < target/.test(fn),
      "этап двигается не «только вперёд» — сделку, ушедшую дальше, откатит назад"
    );
    // Поле обязано пережить нормализацию, иначе защита исчезнет при перезагрузке.
    const norm = app.slice(app.indexOf("function normalizePayment"), app.indexOf("function normalizePayment") + 900);
    assert(
      /portalAdvanceId/.test(norm),
      "normalizePayment теряет portalAdvanceId — после перезагрузки аванс внесётся второй раз"
    );

    /* Обратный ход. Снятие отметки об оплате обязано знать про внесённый
       платёж: иначе в КП аванс не оплачен, а в деньгах сделки он есть — те же
       касса и учёт врозь, только в другую сторону. Спрашиваем, а не удаляем
       молча: отметку снимают и по ошибке, и при возврате. */
    const un = app.slice(app.indexOf("async function unmarkAdvancePaid"), app.indexOf("async function unmarkAdvancePaid") + 1800);
    assert(
      /portalAdvanceId === portalId/.test(un),
      "снятие отметки об оплате не видит внесённое поступление — оно останется висеть"
    );
    assert(
      /confirmDialog\(/.test(un),
      "поступление удаляется молча: у снятия отметки два разных смысла — ошибка и возврат"
    );
  });

  /* Клиент согласовал КП — сделка обязана уйти вперёд по воронке. До 23.08.2026
     подпись жила в client_portals, а сама сделка оставалась на «КП отправлено»:
     на главной висела не в той колонке, а прогноз считал её с весом 0,30 вместо
     0,50 — сделка, по которой клиент уже сказал «да», недооценивалась в деньгах.

     Три условия проверяем именно здесь, потому что цена ошибки в каждом разная:
     движение ТОЛЬКО ВПЕРЁД (иначе откатим сделку, ушедшую дальше), ОДИН РАЗ
     (иначе не дадим студии сознательно вернуть этап) и через _applyCrmStatus
     (присваивание crmStatus напрямую переживёт лишь до следующего
     flushActiveProjectToSaved — статус живёт ещё в snapshot и state.project). */
  await test("согласованное КП двигает сделку на «Согласование» — вперёд и один раз", () => {
    const app = fs.readFileSync(path.join(REPO_ROOT, "app.js"), "utf8");
    const from = app.indexOf("function _syncStagesFromApprovedPortals");
    assert(from > 0, "нет переноса сделки на «Согласование» по подписи клиента");
    const fn = app.slice(from, from + 1800);

    assert(
      /CRM_STATUSES\.indexOf\("Согласование"\)/.test(fn) && /now < target/.test(fn),
      "этап двигается не «только вперёд» — сделку, ушедшую дальше, откатит назад"
    );
    assert(
      /portalApprovedApplied/.test(fn),
      "нет отметки об однократности: студия не сможет вернуть этап — его вернёт обратно следующий заход"
    );
    assert(
      /_applyCrmStatus\(deal, "Согласование"\)/.test(fn),
      "статус присваивается напрямую вместо _applyCrmStatus — его перезапишет snapshot"
    );
    assert(
      /_syncStagesFromApprovedPortals\(\)/.test(app.slice(app.indexOf("async function _loadAllPortals"), app.indexOf("async function _loadAllPortals") + 1500)),
      "перенос не вызывается при загрузке списка КП — значит не сработает никогда"
    );

    /* Обратный ход: снятие согласования обязано сбросить отметку однократности.
       Диалог там прямо обещает «клиент сможет согласовать заново по той же
       ссылке» — без сброса второе согласование сделку бы уже не двинуло, и
       обещание оказалось бы наполовину ложным. */
    const unapprove = app.slice(app.indexOf('title: "Снять согласование?"'), app.indexOf('title: "Снять согласование?"') + 2600);
    assert(
      /portalApprovedApplied = false/.test(unapprove),
      "снятие согласования не сбрасывает отметку однократности — повторная подпись клиента сделку не двинет"
    );

    /* Второй путь к тому же тупику: КП удалили целиком. Ссылку в сделке чистят
       давно, а память о согласовании оставалась — студия отправляла новое КП,
       клиент подписывал, и сделка снова стояла на месте. */
    const del = app.slice(app.indexOf("async function deleteProposal"), app.indexOf("async function deleteProposal") + 2000);
    assert(
      /portalApprovedApplied = false/.test(del),
      "удаление КП не сбрасывает отметку однократности — согласование НОВОГО КП сделку не двинет"
    );
  });
  ...
  /* Красная ЗАЛИВКА — только в окне-подтверждении: там удаление и есть главное
     действие. На странице и в формах разрушительная кнопка стояла таким же
     плотным пятном, как «Сохранить» рядом, и спорила с ней за первый взгляд
     («Очистить смету» в итогах, «Удалить клиента» в окне клиента). Цвет
     предупреждает, вес не кричит: .btn.danger-quiet. */
  await test("разрушительные кнопки: заливка только в окне-подтверждении", () => {
    const src = readSrc("app.js");
    const bad = [];
    /* Строго по списку классов: `icon-danger` — другой компонент (кружок-иконка),
       а строка сборки в confirmDialog («class="btn ' + (danger ? …») не разметка. */
    const re = /class="btn(?: [a-z-]+)*\s+danger(?: [a-z-]+)*"/g;
    let m;
    while ((m = re.exec(src))) {
      const line = src.slice(0, m.index).split("\n").length;
      bad.push(`строка ${line}: ${m[0]}`);
    }
    assertEqual(bad.length, 0,
      "красная заливка вне подтверждения — используйте danger-quiet:\n  " + bad.join("\n  "));
    // Само подтверждение заливку сохраняет — иначе предупреждение исчезнет вовсе.
    assert(/danger \? "danger" : "primary"/.test(src),
      "окно-подтверждение перестало красить кнопку опасного действия");
    assert(/\.btn\.danger-quiet\s*\{/.test(readSrc("style.css")),
      "в стилях нет правила .btn.danger-quiet — класс есть только в разметке");
  });

  /* Эмодзи из интерфейса убрали, но в четырёх местах остались ПУСТЫЕ элементы,
     которым эмодзи задавал размер: `<div style="font-size:48px;margin-bottom:16px">
     </div>` держал дыру в 64px над заголовком пустого состояния, а
     `<span class="help-dd-item-icon" style="background:…"></span>` рисовал шесть
     цветных квадратиков без значка. Пустоту видно только глазами, поэтому ловим
     её здесь: элемент с крупным font-size или с классом значка обязан что-то
     содержать. */
  await test("нет пустых заглушек от убранных эмодзи", () => {
    ...
    while ((m = re.exec(src))) {
      ...
      const cls = m[3] || "";
      // Пустые кружки-подложки размером с текст — это не заглушки, а разделители
      // и точки: придираемся только к крупному кеглю и к слотам под значок.
      if (px < 20 && !cls) continue;
      const line = src.slice(0, m.index).split("\n").length;
      bad.push(`строка ${line}: ${m[0].slice(0, 90)}`);
    }
    assertEqual(bad.length, 0,
      "пустой элемент держит место убранного эмодзи — вставьте icon() или уберите элемент:\n  " + bad.join("\n  "));
  });
  ...
  await test("нет ссылок на несуществующие CSS-переменные", () => {
    ...
    const stripComments = (src) => src.replace(/(?<![\w"'/])\/\*[\s\S]*?\*\//g, (m) => m.replace(/[^\n]/g, " "));
  ...
  /* Письмо со ссылкой на КП — единственный артефакт, который уходит заказчику с
     НАШЕГО домена. Подпись в нём бралась из тела запроса и при пустом профиле
     подставлялась как «Adervis»: заказчик чужой студии получал «КП от Adervis», то
     есть от прямого конкурента. С 18.08 профиль у новых аккаунтов пуст, поэтому
     запасной вариант срабатывал бы всё чаще. Edge Functions прогоном не покрыты
     (нужен Deno и живой Resend), поэтому сторож статический — он ловит именно то,
     что здесь легко вернуть обратно. */
  await test("письмо клиенту: подпись из БД, ответ уходит студии", () => {
    const src = fs.readFileSync(path.join(REPO_ROOT, "supabase/functions/send-portal-email/index.ts"), "utf8");
    assert(!src.includes('|| "Adervis"'),
      "в письме клиенту вернулась подстановка имени сервиса");
    assert(!src.includes("agencyName"),
      "подпись письма снова берётся из тела запроса, а не из agency_state");
    assert(src.includes('from("agency_state")'),
      "письмо больше не читает профиль студии из БД — подписывать его будет нечем");
    assert(src.includes("reply_to"),
      "у письма нет reply_to: ответ заказчика уйдёт на noreply@ и его никто не прочитает");
    assert(!readSrc("app.js").includes("agencyName:"),
      "app.js снова шлёт имя агентства в письмо — сервер обязан брать его из БД");
  });
  ...
  /* Эмодзи из интерфейса убрали, а держатели под них остались: пустые
     <div style="font-size:48px;margin-bottom:16px"></div> и <span> на 18px.
     Ничего не видно, но каждый держит дыру в 30–70px — в брифах пустое
     состояние из-за такой дыры выглядело иначе, чем во всех прочих разделах.
     Ловим по признаку «пустой элемент, которому задан крупный кегль». */
  await test("нет пустых держателей от убранных эмодзи", () => {
    const code = readSrc("app.js");
    const bad = [];
    for (const m of code.matchAll(/<(div|span|p)\b[^>]*style="([^"]*)"[^>]*>\s*<\/\1>/g)) {
      const size = /font-size:\s*(\d+)px/.exec(m[2]);
      if (!size || Number(size[1]) < 16) continue;
      bad.push("строка " + code.slice(0, m.index).split("\n").length + ": " + m[0].slice(0, 90));
    }
    assert(!bad.length,
      "пустые элементы с крупным кеглем — держат отступ и ничего не показывают:\n  " + bad.join("\n  "));
  });

  /* Плюс в ПОДПИСИ кнопки («+ Задача», «+ Участник») делал ту же работу, что
     значок, но не выравнивался по сетке: в ряду с кнопками, у которых значок
     настоящий, подписи стояли на разной высоте. Значок несёт «добавить», слово —
     «что». Исключение одно: пара «+ Поступление» / «− Расход», где знак означает
     направление денег, а не действие. */
  await test("кнопки не подписаны плюсом вместо значка", () => {
    const code = readSrc("app.js");
    const bad = [];
    for (const m of code.matchAll(/>\+\s[А-ЯA-Zа-яa-z][^<]{2,30}</g)) {
      if (/\+\s*Поступление/.test(m[0])) continue; // пара со смыслом
      bad.push("строка " + code.slice(0, m.index).split("\n").length + ": " + m[0].slice(1, -1).trim());
    }
    for (const m of code.matchAll(/label:\s*"\+\s[^"]+"/g)) {
      bad.push("строка " + code.slice(0, m.index).split("\n").length + ": " + m[0]);
    }
    assert(!bad.length,
      "плюс в подписи кнопки вместо значка (icon(\"plus\")):\n  " + bad.join("\n  "));
  });

  await test("тексты продвижения не расходятся с ценой и триалом", () => {
    /* У цены теперь ТРИ копии, а не две: витрина (PLANS), касса (Edge Function
       create-payment) и тексты, которые владелец копирует в соцсети и в письма
       клиентам. Третья — самая опасная: она уезжает наружу и живёт там месяцами,
       а разойтись может молча. Ровно так протух скриншот тарифов (11.08.2026),
       где слайд показывал цены до подорожания.
    ...
    // Длина триала: в текстах обещаем ровно то, что ставит триггер в базе.
    const дни = (readSrc("supabase/migrations/20260820000003_fix_protect_subscription_security_definer.sql")
      .match(/interval\s+'(\d+)\s+days'/) || [])[1];
    assert(дни, "не нашёл длину триала в миграции");
    const обещано = [...блок.matchAll(/(\d+)\s+дн[еяй]+\s+бесплатно/g)].map(m => m[1]);
    assert(обещано.length, "в текстах не обещан бесплатный период");
    for (const d of обещано) {
      assertEqual(d, дни, `в тексте обещано ${d} дней триала, а база даёт ${дни}`);
    }

    // id промптов уникальны — иначе «Копировать» отдаст не тот текст.
    const ids = [...блок.matchAll(/^\s*id:\s*"([\w-]+)"/gm)].map(m => m[1]);
    assertEqual(ids.length, new Set(ids).size, "дублируются id в текстах продвижения: " + ids.join(", "));
  });

  await test("касса не выставляет счёт на ноль по промокоду 100%", () => {
    /* Схема разрешает промокод на 100% (CHECK BETWEEN 1 AND 100), и это
       осмысленно — «бесплатно амбассадору». Но счёт на ноль ЮKassa отклоняет, и
       до 04.09.2026 человек получал невнятное «Ошибка платёжной системы: {...}»:
       отказ приходил от чужого сервиса, и связать его с промокодом было нельзя.

       Сторож на ИСХОДНИК, а не на поведение: Deno в офлайновом прогоне не
       поднять, а проверка стоит ровно перед вызовом кассы, и её удаление вернёт
       ту же невнятицу. */
    const ef = readSrc("supabase/functions/create-payment/index.ts");
    const i = ef.indexOf("finalAmount");
    assert(i > 0, "в кассе не найден расчёт итоговой суммы");
    const хвост = ef.slice(i, ef.indexOf("api.yookassa.ru"));
    assert(хвост.length > 0, "не найден вызов ЮKassa после расчёта суммы");
    assert(/finalAmount\s*<=\s*0/.test(хвост),
      "перед вызовом кассы нет проверки на нулевой счёт — промокод на 100% вернёт невнятную ошибку платёжной системы");
    assert(/админк|Активировать подписку/i.test(хвост),
      "проверка есть, но не говорит, ЧТО делать вместо неё — а решение принимает владелец, а не клиент");
  });

  await test("список «сделка доведена» не размножается по файлу", () => {
    /* На 04.09.2026 набор статусов «Сдано / Оплата / Завершённые / Архив» был
       написан руками в ПЯТИ местах, при том что рядом жил именованный
       `DEAL_DELIVERED`. Одна из копий уже разъехалась (у среднего чека нет
       «Архива»), и заметить это можно было только чтением всех пяти подряд.

       Ровно так на этом проекте расходились цена (витрина ↔ касса) и правила
       дедлайнов на соседних числах одной плитки. Сторож держит одно определение.

       Исключения названы поимённо, а не «первые три вхождения»: если появится
       шестое место, тест обязан упасть, а не молча пропустить его. */
    const code = readSrc("app.js").replace(/(?<![\w"'/])\/\*[\s\S]*?\*\//g, "").replace(/^[ \t]*\/\/[^\n]*$/gm, "");
    const РАЗРЕШЕНО = [
      "CRM_STATUSES",    // полная воронка, порядок статусов — другая сущность
      "DEAL_DELIVERED",  // канонический набор «работа доведена»
      "closedDeals",     // НАМЕРЕННО у́же: «Архив» — отменённая сделка, не продажа
    ];
    const плохо = [];
    // Любой литерал массива/множества, где рядом стоят все три статуса.
    for (const m of code.matchAll(/\[[^\]\n]*"Сдано"[^\]]*\]|\[[^\]\n]*"Завершённые"[^\]]*\]/g)) {
      const кусок = m[0];
      if (!/"Сдано"/.test(кусок) || !/"Оплата"/.test(кусок) || !/"Завершённые"/.test(кусок)) continue;
      const строка = code.slice(0, m.index).split("\n").length;
      const рядом = code.slice(Math.max(0, m.index - 160), m.index);
      if (РАЗРЕШЕНО.some(x => рядом.includes(x))) continue;
      плохо.push(`строка ${строка}: ${кусок.slice(0, 70)}`);
    }
    assert(!плохо.length,
      "набор статусов «сделка доведена» написан руками мимо DEAL_DELIVERED:\n  " + плохо.join("\n  ") +
      "\n  Используйте DEAL_DELIVERED.has(...) — иначе копии разъедутся молча.");
  });

  await test("свободный текст не уезжает в инлайновый обработчик", () => {
    /* Метка аккаунта (`admin_tag`) — свободный ввод. Если подставить её прямо в
       onclick, апостроф ломает кнопку МОЛЧА: `escapeHtml` превращает `'` в
       `&#039;`, HTML-разборщик возвращает его в `'` ДО выполнения JS, и
       `app._setAdminUsersTag('Свой'аккаунт')` становится синтаксической ошибкой.
       Ни исключения в консоли при рендере, ни визуального признака — просто
       мёртвая кнопка у того, кто ввёл апостроф.

       Поэтому в обработчик уходит ПОЗИЦИЯ в списке, а не текст. Тест держит
       именно это: аргумент не может быть строкой в кавычках. */
    /* Комментарии вырезаем: в них РАЗБИРАЕТСЯ, как именно ломается кнопка, и
       пример сломанного вызова стоит там законно — на нём тест сам себя и
       уронил при первом прогоне. Вырезаем оба вида, блочные тоже: разбор
       написан блочным. */
    // Оговорка про «/*» после буквы — см. сторож «свободный текст не попадает в
    // код обработчика»: без неё accept="image/*" съедал десятки строк кода.
    const code = readSrc("app.js")
      .replace(/(?<![\w"'/])\/\*[\s\S]*?\*\//g, (c) => c.replace(/[^\n]/g, ""))
      .replace(/^[ \t]*\/\/[^\n]*$/gm, "");
    const плохо = [];
    for (const m of code.matchAll(/_setAdminUsersTag\(\s*['"`]/g)) {
      плохо.push("строка " + code.slice(0, m.index).split("\n").length);
    }
    assert(!плохо.length,
      "в _setAdminUsersTag передаётся строка, а не позиция — апостроф в метке убьёт кнопку:\n  " + плохо.join("\n  "));
  });
  ...
  await test("версия в приложении совпадает с номером сборки в sw.js", () => {
    /* «4.3» стояло в APP_VERSION с 01.07.2026 и пережило 330 коммитов: на экране
       ...
       значило. Второй ручной счётчик заводить незачем — номер сборки уже есть в
       CACHE_NAME, поднимается на каждый выпуск и проверяется CI. Тест держит их
       в согласии: забыл поднять APP_BUILD — прогон красный. */
    const app = readSrc("app.js");
    const sw = readSrc("sw.js");
    const build = (app.match(/APP_BUILD\s*=\s*(\d+)/) || [])[1];
    const cache = (sw.match(/CACHE_NAME\s*=\s*"adervis-crm-v(\d+)"/) || [])[1];
    assert(build, "в app.js не найден APP_BUILD");
    assert(cache, "в sw.js не найден номер в CACHE_NAME");
    assertEqual(build, cache,
      `версия приложения (APP_BUILD=${build}) разошлась с номером сборки (sw.js v${cache}) — ` +
      "поднимите APP_BUILD вместе с CACHE_NAME");
  });

  await test("витрина не называет налоговый статус продавца", () => {
    /* Решение владельца 03.09.2026: «Самозанятый» звучит мельче продукта, в
       интерфейсе его быть не должно. Статус остаётся в ОФЕРТЕ и договоре — там
       он обязателен по закону; продавца в витрине идентифицирует ИНН.

       Проверяем только те куски, которые видит покупатель: подвал настроек и
       реквизиты в профиле. Тексты оферты и шаблона договора не трогаем. */
    const code = readSrc("app.js");
    const подвал = code.slice(code.indexOf("ADERVIS · сметы и КП для видеопродакшна"));
    assert(подвал, "подвал с реквизитами пропал");
    assert(!/Самозанят/i.test(подвал.slice(0, 400)),
      "налоговый статус вернулся в подвал приложения");
    const i = code.indexOf(">Реквизиты<");
    assert(i > 0, "блок «Реквизиты» пропал из профиля");
    assert(!/Самозанят/i.test(code.slice(i, i + 900)),
      "налоговый статус вернулся в реквизиты профиля");
    // Но в оферте он обязан остаться — иначе документ станет недействительным.
    assert(/Самозанятого\) Никитина/.test(code),
      "из оферты пропал налоговый статус — он там обязателен по закону");
  });

  await test("«Опасная зона» не обещает, что сброс останется в браузере", () => {
    /* Сброс ставит пустое состояние и зовёт save(), а save() через три секунды
       отправляет его в agency_state — облачная копия обнуляется тоже, и данные
       пропадают на всех устройствах. До 03.09.2026 экран писал «удалит все
       локальные данные приложения в браузере», а диалог — «настройки этого
       устройства»: человек мог нажать «Сбросить» ИМЕННО потому, что считал
       облако запасной копией.

       Сторож на слова, а не на поведение: если однажды сброс сделают правда
       локальным, текст придётся править вместе с ним — и тест об этом напомнит. */
    const code = readSrc("app.js");
    const i = code.indexOf("Опасная зона");
    assert(i > 0, "блок «Опасная зона» пропал из настроек");
    const блок = code.slice(i, i + 1200);
    assert(/облак/i.test(блок),
      "«Опасная зона» не предупреждает, что сброс затрёт и облачную копию");
    const j = code.indexOf("Сбросить все данные?");
    assert(j > 0, "пропал диалог подтверждения сброса");
    assert(/облак/i.test(code.slice(j, j + 600)),
      "диалог сброса не предупреждает про облако");
    assert(!/локальные данные приложения в браузере/.test(code),
      "вернулась формулировка «локальные данные в браузере» — она неправда, пока сброс идёт через save()");
  });

  await test("плюс-значок стоит только у кнопок, названных существительным", () => {
    /* Решение владельца 03.09.2026: «"+" убираем где он не нужен, это как будто
       паттерн ИИ». Правило, которое из этого вышло и которое стоит держать:

         плюс к СУЩЕСТВИТЕЛЬНОМУ — да («Своя позиция», «Свой пакет»): подпись
           называет сущность, а действие «создать» несёт как раз значок;
         плюс к ГЛАГОЛУ — нет («Добавить», «Создать промокод»): значок повторяет
           слово. Ровно за это же 31.08 убрали шаблонные «→» с кнопок-глаголов.

       Отдельный довод для карточки каталога: в ТОЙ ЖЕ точке кнопка превращается
       в счётчик «− 1 +», где плюс значит «ещё одну». Один знак в одном месте не
       может означать то украшение, то действие. */
    const code = readSrc("app.js");
    // Глаголы, после которых плюс — повтор. Список, а не «всё кроме разрешённых»:
    // подписей-существительных два десятка, и держать их перечнем значит ломать
    // прогон на каждой новой кнопке.
    const ГЛАГОЛЫ = /^(Добав|Создать|Сохран|Внести|Завести|Оформ|Написа|Загруз|Выгруз|Отправ)/i;
    const bad = [];
    /* Плюс рисуется ДВУМЯ способами, и первая версия сторожа знала только один:
       вставленный <svg> с путём плюса и общий помощник icon("plus"). Из-за этого
       мимо прошла кнопка «+ Добавить» в шапке сметы. Проверяем оба. */
    const формы = [
      /<svg[^>]*>\s*<path d="M12 5v14M5 12h14"\/>\s*<\/svg>\s*([^<\n]{0,40})/g,
      /icon\("plus"[^)]*\)\}\s*([^<\n`]{0,40})/g,
    ];
    for (const re of формы) {
      let m;
      while ((m = re.exec(code))) {
        const label = m[1].trim();
        if (!label) continue;                 // значок без подписи — иконочная кнопка, это законно
        if (!ГЛАГОЛЫ.test(label)) continue;   // существительное — плюс несёт «создать», оставляем
        bad.push("строка " + code.slice(0, m.index).split("\n").length + ": «" + label + "»");
      }
    }
    assert(!bad.length,
      "плюс-значок рядом с подписью-глаголом — подпись уже говорит это слово:\n  " + bad.join("\n  ") +
      "\n  Плюс оставляем только там, где подпись — существительное («Своя позиция», «Задача на дату»).");
  });

  await test("свободный текст не попадает в код обработчика on…=", () => {
    /* escapeHtml защищает АТРИБУТ, но не JS-строку внутри него: разборщик HTML
       возвращает &#39; в апостроф до того, как код попадёт в движок. Поэтому
       onclick="app.f('${escapeHtml(текст)}')" выглядит обезвреженным, а на деле
       мёртв при апострофе и исполняет текст-ловушку. На этом ловились трижды:
       метка аккаунта в админке (04.09), теги сделок и категория договора (11.09),
       почта в кнопке «Возврат» (11.09 — почту пишет себе сам посторонний человек,
       а жмёт суперадмин).

       Правило: в обработчик — id, позицию или ключ из закрытого набора; текст —
       в data-* и читать как this.dataset.x.

       Два перечня намеренно разной природы. Для escapeHtml — СПИСОК РАЗРЕШЁННОГО:
       именно эта форма создаёт ложное чувство безопасности, и каждое новое
       значение пусть проходит через осознанное решение. Для сырой подстановки —
       список запретных ИМЁН полей: там в основном id и константы, и держать их
       перечнем значило бы ломать прогон на каждой новой кнопке.

       Сторож выше держит один частный случай (_setAdminUsersTag), этот — весь
       класс. 04.09 частный случай закрыли, а остальные не поискали — и 11.09 их
       нашлось ещё шесть. */
    // Комментарии вырезаем, как и сторож выше: в пояснениях законно стоит пример
    // сломанного вызова. Переводы строк внутри оставляем — иначе номера строк в
    // сообщении разъедутся с файлом.
    //
    // «/*» перед буквой, кавычкой или слэшем — НЕ комментарий: accept="image/*"
    // открывал «комментарий» до ближайшего «*/», и из проверки молча выпадало
    // 18 живых обработчиков (профиль, валюта, статьи финансов). Замер 11.09:
    // с этой оговоркой из 852 обработчиков уходит ровно один — тот, что в
    // настоящем комментарии. И [ \t]*, а не \s*: \s глотал пустые строки перед
    // «//», и номера строк уезжали на 342.
    const code = readSrc("app.js")
      .replace(/(?<![\w"'/])\/\*[\s\S]*?\*\//g, (c) => c.replace(/[^\n]/g, ""))
      .replace(/^[ \t]*\/\/[^\n]*$/gm, "");
    const РАЗРЕШЕНО = [
      /(^|\.)id$/, /Id$/, /^String\(a\.id\)$/,
      /^feedUrl$/,                    // адрес фида собирает приложение
      /^(ev|event)\.htmlLink$/,       // ссылка из Google Календаря
      /^status$/,                     // колонки канбана — CRM_STATUSES
      /^m\.label$/,                   // подпись месяца на графике
      /^ast$/, /^a\.subscription_plan\|\|""$/,   // статус и тариф — закрытые наборы в БД
    ];
    const ТЕКСТОВЫЕ_ПОЛЯ = /\.(name|title|email|tags?|label|text|note|notes|comment|desc|category|goodFor|author|phone|company)$/;

    /* Тело атрибута ищем посимвольно, а ${…} проходим ЦЕЛИКОМ. Первая версия брала
       /on[a-z]+="([^"]*)"/ — и обрезала обработчик на первой кавычке, даже если та
       стоит внутри выражения: onclick="app.adminRefund('${aid}','${escapeHtml(a.email||"")}')"
       кончался на a.email|| , подстановка не находилась, и сторож пропускал ровно
       самый опасный случай (почту пишет себе посторонний, а жмёт суперадмин). */
    const bodies = [];
    for (const st of code.matchAll(/\bon[a-z]+="/g)) {
      const from = st.index + st[0].length;
      let i = from, depth = 0;
      for (; i < code.length && i - from < 3000; i++) {
        const c = code[i];
        if (depth === 0) {
          if (c === '"') break;
          if (c === "$" && code[i + 1] === "{") { depth = 1; i++; }
        } else if (c === "{") depth++;
        else if (c === "}") depth--;
      }
      bodies.push({ line: code.slice(0, st.index).split("\n").length, body: code.slice(from, i) });
    }
    assert(bodies.length > 500, `нашлось всего ${bodies.length} обработчиков — разбор сломался и смотрит мимо`);

    const bad = [];
    for (const { line, body } of bodies) {
      for (const k of body.matchAll(/'\$\{escapeHtml\((.*?)\)\}'/g)) {
        const expr = k[1].trim();
        if (!РАЗРЕШЕНО.some((r) => r.test(expr))) bad.push(`строка ${line}: escapeHtml(${expr})`);
      }
      for (const k of body.matchAll(/'\$\{([^}]*)\}'/g)) {
        const expr = k[1].trim();
        if (!/^escapeHtml\(/.test(expr) && ТЕКСТОВЫЕ_ПОЛЯ.test(expr)) bad.push(`строка ${line}: \${${expr}} без всякого экранирования`);
      }
    }
    assert(!bad.length,
