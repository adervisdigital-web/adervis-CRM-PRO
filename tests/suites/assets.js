...
module.exports = async function ({ test }) {
  ...
  await test("мобильное меню содержит ВСЕ разделы из SIDEBAR_NAV_DEFS", () => {
    // Список разделов задан один раз в SIDEBAR_NAV_DEFS, а меню «Ещё» написано
    // руками — и разошлось: «Все КП» и «Команда» в него не попали, то есть с
    // телефона в эти разделы было не попасть вовсе. Сверяем по ИДЕНТИФИКАТОРАМ,
    // а не по подписям: в сайдбаре раздел называется «Договора», в меню
    // «Договоры», и придираться к формулировке тут не за чем.
    ...
    // Ищем по шаблону САМОГО пункта, а не по вырезанному куску разметки: любой
    // пункт этого меню сперва закрывает меню, а потом переходит — комбинация
    // встречается только здесь и не зависит от того, чем блок начинается и кончается.
    const missing = ids.filter(id => !app.includes(`app.closeMainMenu();app.go('${id}')`));
    assert(missing.length === 0,
      "разделы есть в SIDEBAR_NAV_DEFS, но недостижимы из меню на телефоне: " + missing.join(", "));
  });

  await test("логотип ничего не дублирует, меню открывает только «Ещё»", () => {
    // Логотип успел побывать кнопкой меню (дублировал «Ещё») и переходом на главную
    // (дублировал «Проекты»). На телефоне вся навигация лежит в нижней панели, так
    // что ЛЮБОЕ действие на логотипе повторяет одну из её кнопок — поэтому он просто
    // фирменный знак: ни обработчика, ни role="button", ни фокуса.
    const logo = index.slice(index.indexOf('<div class="logo"'), index.indexOf(">", index.indexOf('<div class="logo"')));
    assert(!/onclick/.test(logo), "на логотипе снова появилось действие — оно дублирует кнопку нижней панели");
    assert(!/role="button"|tabindex/.test(logo), "логотип снова притворяется кнопкой, ничего не делая");
    // А «Ещё» — наоборот, обязана открывать меню: другого входа в него нет.
    const more = index.slice(index.indexOf('id="mbnMore"'), index.indexOf(">", index.indexOf('id="mbnMore"')));
    assert(/openMainMenu/.test(more), "кнопка «Ещё» перестала открывать меню — разделы стали недостижимы");
  });
  ...
  await test("выход из аккаунта не продублирован в списке разделов", () => {
    // Выход живёт в меню профиля в шапке. В списке разделов он был лишним способом
    // случайно закрыть себе сессию.
    assert(!/app\.closeMainMenu\(\);app\.adminLogout\(\)/.test(app),
      "«Выйти» снова в мобильном меню разделов — выход уже есть в меню профиля");
  });
