...
module.exports = async function ({ test }) {
  ...
  await test("логотип открывает разделы, кнопка панели — настройку меню", () => {
    // Две кнопки — две РАЗНЫЕ задачи (решение владельца). Логотип разворачивает
    // список разделов, а кнопка в нижней панели настраивает этот список: какие
    // разделы показывать и в каком порядке. Раньше обе открывали одно и то же.
    const logo = index.slice(index.indexOf('<div class="logo"'), index.indexOf(">", index.indexOf('<div class="logo"')));
    assert(/app\.openMainMenu\(\)/.test(logo), "логотип перестал открывать меню разделов");
    ...
    assert(/toggleSidebarNavPopover/.test(more), "кнопка нижней панели снова открывает меню разделов вместо его настройки");
    assert(!/openMainMenu/.test(more), "в нижней панели остался второй вход в меню разделов — он дублирует логотип");
    // Подпись обязана называть действие: «Ещё» при открытии настроек — обман.
    assert(/Настроить/.test(more), "подпись кнопки не соответствует тому, что она открывает");
  });
