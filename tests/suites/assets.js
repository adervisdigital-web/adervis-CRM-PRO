...
module.exports = async function ({ test }) {
  ...
  await test("логотип ведёт на главную, а меню открывает только «Ещё»", () => {
    // Логотип и кнопка «Ещё» делали одно и то же, причём логотип — вопреки привычке:
    // он везде возвращает на главный экран, а не разворачивает список разделов.
    const logo = index.slice(index.indexOf('class="logo"'), index.indexOf(">", index.indexOf('class="logo"')));
    assert(!/openMainMenu/.test(logo), "логотип снова открывает меню вместо перехода на главную");
    assert(/app\.go\('home'\)/.test(logo), "логотип никуда не ведёт");
