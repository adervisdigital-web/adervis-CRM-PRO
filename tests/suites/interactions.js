...
module.exports = async function ({ browser, baseUrl, test }) {
  ...
  /* Под сеткой месяца счётчик писал «Всего событий: 2», а строкой ниже, над тем
  ...
  await test("календарь: счётчики под сеткой и над списком показывают одно число", async () => {
    ...
    try {
      ...
      await own.page.waitForTimeout(600);
      const счётчики = await own.page.evaluate(() => {
      ...
      счётчики.forEach((s) => {
        ...
        const хвост = n % 100 >= 11 && n % 100 <= 14 ? "событий"
          : n % 10 === 1 ? "событие"
          : n % 10 >= 2 && n % 10 <= 4 ? "события" : "событий";
        assert(s.endsWith(хвост), `«${s}» — форма слова не по числу, ожидалось «${n} ${хвост}»`);
      });
    } finally {
      await own.context.close();
    }
