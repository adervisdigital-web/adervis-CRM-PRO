...
serve(async (req) => {
  ...
  if (!agencies?.length) return new Response(JSON.stringify({ notified: 0, agencies: 0 }));
  ...
  for (const agency of agencies) {
    ...
    // Find active deals with deadline today or tomorrow
    const upcoming = projects.filter((p) => {
      if (!p.deadline) return false;
      if (["Завершённые", "Сдано"].includes(p.crmStatus || "")) return false;
  ...
  return new Response(
    JSON.stringify({ notified, agencies: agencies.length }),
